...
export default function IsHealthySearch({ initialProducts: _ }: Props) {
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div className="flex items-center gap-3 bg-white rounded-2xl border border-[#dad7cc] px-5 py-4 shadow-sm focus-within:border-[#2e7d32] focus-within:ring-2 focus-within:ring-[#2e7d32]/10 transition-all">
        {loading ? (
          <svg className="w-5 h-5 text-[#2e7d32] animate-spin shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
          </svg>
        ) : (
          <svg className="w-5 h-5 text-[#9ca3af] shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        <input
          ...
          placeholder="Search any food, supplement or cosmetic..."
            ...
            >
              <div className="w-11 h-11 rounded-xl bg-[#f2f0e9] shrink-0 overflow-hidden flex items-center justify-center">
                {product.product_image_url ? (
                  <Image src={product.product_image_url} alt={displayName(product)} width={44} height={44} unoptimized className="w-full h-full object-contain" />
                ) : (
